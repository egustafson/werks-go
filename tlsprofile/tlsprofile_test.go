...
func (s *TLSProfileTestSuite) TestTLSProfile() {
	ca, caKey := tlsprofile.MakeCertAndKey()
	cert, key := tlsprofile.MakeCertAndKey(tlsprofile.WithSigner(ca, caKey))
	...
	var profileTests = []struct {
		n string
		p *tlsprofile.TLSProfile
	}{
		{"DisableTLS", &tlsprofile.TLSProfile{DisableTLS: true}},
		{"empty PEMS", &tlsprofile.TLSProfile{ /* enabled tls, empty PEMS */ }},
		{"only CA", &tlsprofile.TLSProfile{
			CA: s.validPEM.ca,
		}},
		{"only cert/key", &tlsprofile.TLSProfile{
			Cert: s.validPEM.cert,
			Key:  s.validPEM.key,
		}},
		{"everything", &tlsprofile.TLSProfile{
			CA:   s.validPEM.ca,
			Cert: s.validPEM.cert,
			Key:  s.validPEM.key,
		}},
		{"everything+InsecureSkipVerify", &tlsprofile.TLSProfile{
			CA:                 s.validPEM.ca,
			Cert:               s.validPEM.cert,
			Key:                s.validPEM.key,
			InsecureSkipVerify: true,
		}},
		{"file ca only", &tlsprofile.TLSProfile{
			CA: s.validFiles.ca,
		}},
		{"file cert/key only", &tlsprofile.TLSProfile{
			Cert: s.validFiles.cert,
			Key:  s.validFiles.key,
		}},
		{"file everything", &tlsprofile.TLSProfile{
			CA:   s.validFiles.ca,
			Cert: s.validFiles.cert,
			Key:  s.validFiles.key,
		}},
...
func (s *TLSProfileTestSuite) TestTLSProfile_InsecureSkipVerify() {
	TLSProfileInsecureSkipVerify := &tlsprofile.TLSProfile{
		CA:                 s.validPEM.ca,
		Cert:               s.validPEM.cert,
		Key:                s.validPEM.key,
		InsecureSkipVerify: true,
	...
	TLSProfileYesAuth := &tlsprofile.TLSProfile{
		CA:                 s.validPEM.ca,
		Cert:               s.validPEM.cert,
		Key:                s.validPEM.key,
...
func (s *TLSProfileTestSuite) TestTLSProfile_NonExistantPath() {
	...
	TLSProfile := &tlsprofile.TLSProfile{
	...
	profile2 := &tlsprofile.TLSProfile{
		CA:   s.validFiles.ca,
	...
	profile3 := &tlsprofile.TLSProfile{
		CA:   s.validFiles.ca,
		Cert: s.validFiles.cert,
		Key:  "more-garbage",
	}
...
func (s *TLSProfileTestSuite) SetupSuite() {
	...
	// -- ValidCertChainPEM --
	var chainLen uint = 4
	chain := tlsprofile.MakeCertChain(tlsprofile.WithLength(chainLen))
	certChainPEM := ""
	...
	// -- EarlyInvalidPEM --
	cert, key = tlsprofile.MakeCertAndKey(
		tlsprofile.WithSigner(ca, caKey),
		tlsprofile.WithNotBefore(time.Now().Add(10*time.Minute)),
	)
	...
	cert, key = tlsprofile.MakeCertAndKey(
		tlsprofile.WithSigner(ca, caKey),
		tlsprofile.WithNotAfter(time.Now().Add(-time.Minute)),
	)
